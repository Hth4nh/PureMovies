...
function replaceLogo() {
    ...
    window.logo.style.transform = `rotate(${DEBUG*180}deg) scaleY(${DEBUG*-2 + 1}) scaleX(${DEBUG*-2 + 1})`;
    window.logo.style.filter = `invert(${DEBUG*1})`;
}
...
function initEpsListOnClick(parentQuery) {
    ...
    if (!window.scrollToHereWrapper) {
        ...
        scrollToHere.style.borderBottom = (DEBUG) ? "5px dotted red" : "";
...
(async function main() {
    ...
    GM_addValueChangeListener("DEBUG", (key, oldValue, newValue, remote) => {
        ...
        replaceLogo();
        if (!window.scrollToHere) {
            window.scrollToHere.style.border = (DEBUG) ? "5px dotted red" : "";
        }
        Object.keys(blobURLMap).forEach(key => delete blobURLMap[key]);
    });
...
document.addEventListener("keydown", (event) => {
    window.typed ??= "";
    window.typed = window.typed.slice(-99) + event.key.toUpperCase();

    if (window.typed.endsWith("FLASH")) {
        FLASH = !FLASH;
    ...
    if (window.typed.endsWith("DEBUG")) {
