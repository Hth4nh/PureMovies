...
async function convertToBlobURL(_url) {
    ...
    // Remove ads
    res = res.replaceAll(/(?<!#EXT-X-DISCONTINUITY[\s\S]*)#EXT-X-DISCONTINUITY\n(?:.*?\n){20}#EXT-X-DISCONTINUITY\n(?![\s\S]*#EXT-X-DISCONTINUITY)/g, '');
    //res = res.replaceAll(/#EXT-X-DISCONTINUITY\n(?:.*\n){18}#EXT-X-DISCONTINUITY\n(?:.*\n){4}(?=#EXT-X-DISCONTINUITY)/g, '');
    //res = res.replaceAll(/#EXT-X-DISCONTINUITY\n(?:.*\n){18}(?=#EXT-X-DISCONTINUITY)/g, '');
...
function initIframe(parentQuery = "body", callbackFn = () => {}) {
    ...
    injectPlayer(window.container, parentQuery, callbackFn);
}
...
(async function main() {
    ...
    let logoAlt = "Cuki's PureMovies";

    document.querySelectorAll("img[src*=ogo]").forEach(img => {
        img.src = logoURL;
        img.alt = logoAlt;
